...
#N_A = 1452
#t   = 51300
#N_4 = 5 # он не задан в примере икд
#i_sr                  =  64.8 #[deg] 
#T_sr                  = 40544.0 #[с] 

# интервал прогноза
N_A     = 387 # номер суток внутри 4-х летнего периода относительно даты полуения альманаха
...
i                     = ((i_sr / 180.0) + alm.dI) * math.pi

# сред. драконич. период на витке W+1 и сред. движение
T_dr                  = T_sr + alm.dT + (((2.0 * W) + 1.0) * alm.dTT)
...
J_2_0                 = (1082.62575) * (10 ** (-6.0))
 
GM                    = (398600441.8) * (10 ** (6.0))
...
while abs(a_new - a) > 0.01:
    
    a                 = a_new
    
    T_osk             = T_osk_new
    
    a_new             = ((((T_osk)/(2.0 * math.pi)) ** (2.0)) * GM) ** (1./3)
    
    p_new             = a_new * (1.0 - ((alm.E) ** (2.0)))
    
    znam_p1           = ((3.0 / 2.0) * J_2_0 * ((a_e) / (p_new)) ** (2.0)) 
    
    znam_p2_1         = (2.0 - (5.0 / 2.0) * (math.sin(i) ** (2.0))) * \
                        (((1.0 - (alm.E ** (2.0))) ** (3./2)) / ((1.0 + alm.E * math.cos(alm.w_a * math.pi)) ** (2.0)))    
    
    znam_p2_2         = ((1.0 + alm.E * math.cos(alm.w_a * math.pi)) ** (3.0)) / (1.0 - (alm.E ** (2.0)))
...
w_arg_per             = alm.w_a * math.pi - ((3.0 / 4.0) * J_2_0 * n * ((a_e / p_new) ** (2.0)) * (1.0 - 5.0 * math.cos(i) ** (2.0))) * delta_t_pr

# значение средней долготы на момент прохождения текущего восходящего узла
E_0                   = -2.0 * math.atan((((1.0 - alm.E) / (1.0 + alm.E)) ** (1. / 2))  * math.tan(w_arg_per /2.0))
L_1                   = w_arg_per + E_0 - alm.E * math.sin(E_0)

# текущее значение средней долготы НКА
L                     = L_1 + (n * (delta_t_pr - ((T_sr + alm.dT) * W) - (alm.dTT * (W ** (2.0)))))
...
v                     = 2.0 * math.atan((((1.0 - alm.E) / (1.0 - alm.E)) ** (1.0 / 2)) * math.tan(E_new / 2.0))
u                     = v + w_arg_per

# координаты центра масс НКА в ECEF(?)
p                     = a_new * (1.0 - (alm.E ** (2.0)))
r                     = (p) / (1.0 + alm.E * math.cos(v))
...
# скорость центра масс НКА
EARTH_MU = 3.986004418e14

v_r      = ((EARTH_MU / p_new) ** (1./2)) * alm.E * math.sin(v)
v_u      = ((EARTH_MU / p_new) ** (1./2)) * (1 + alm.E * math.cos(v))


v_x_ti   = v_r * (math.cos(lam) * math.cos(u) - math.sin(lam) * math.sin(u) * math.cos(i)) - \
         v_u * (math.cos(lam) * math.sin(u) + math.sin(lam) * math.cos(u) * math.cos(i)) + \
         w_z * y_ti
         
v_y_ti   = v_r * (math.sin(lam) * math.cos(u) + math.cos(lam) * math.sin(u) * math.cos(i)) - \
         v_u * (math.sin(lam) * math.sin(u) - math.cos(lam) * math.cos(u) * math.cos(i)) -  \
         w_z * x_ti
         
v_z_ti   = v_r * math.sin(u) * math.sin(i) + v_u * math.cos(u) * math.sin(i)
...
E                     = L - w_arg_per
E_new                 = E * 1.01
while abs(E_new - E) >= (10 ** (-9.0)):
    E                 = E_new
    E_new             = L - w_arg_per + alm.E * math.sin(E)

#  истинная аномалия v и аргумент широты НКА u
v                     = 2.0 *  math.atan((((1.0 - alm.E) / (1.0 - alm.E)) ** (1.0 / 2.0 )) * math.tan(E_new / 2.0))
u                     = v + w_arg_per
